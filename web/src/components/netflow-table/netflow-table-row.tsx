...
const NetflowTableRow: React.FC<{
  ...
  highlight: boolean;
  tableWidth: number;
  computedHeightClass?: string;
}> = ({ flow, selectedRecord, columns, size, onSelect, highlight, tableWidth, computedHeightClass }) => {
  const onRowClick = () => {
    onSelect(flow);
  };
    ...
    <Tr isRowSelected={flow.key === selectedRecord?.key} onRowClick={onRowClick}>
      {columns.map(c => (
        <CSSTransition
          ...
          classNames="newflow"
        >
          <Td key={c.id} className={computedHeightClass} style={{ width: `${Math.floor((100 * c.width) / tableWidth)}%` }}>
