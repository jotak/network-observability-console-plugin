...
import { elementPerMinText } from '../utils/count';
...
const getStatForDisplay = (stats: MetricStats, mt: MetricType, mf: MetricFunction): string => {
  ...
  return getValueForDisplay(stat, mt, mf);
};

export const getValueForDisplay = (v: number, mt: MetricType, mf: MetricFunction): string => {
  switch (mt) {
    case 'bytes':
